...
def test_dashboard_includes_external_environment_summary():
    service = ExecutiveDashboardService()

    environment_stub = Mock()
    environment_stub.pest.economic = 0.45
    environment_stub.competitors = [Mock(aggressiveness=0.6), Mock(aggressiveness=0.4)]
    environment_stub.shocks = [Mock(shock_type='recession'), Mock(shock_type='currency')]

    with patch('app.services.executive_dashboard_service.ExternalEnvironmentServiceV2') as mocked_service, \
         patch.object(service, 'culture_service') as mock_culture, \
        ...
        mocked_service.return_value.get_environment.return_value = environment_stub
        mock_culture.get_latest_culture.return_value = None
        ...
        mock_learning.get_latest_persona.return_value = AICeoPersona(
            aggressiveness=0.5,
            risk_tolerance=0.5,
            brand_priority=0.5,
    ...
    assert dashboard.environment is not None
    assert dashboard.environment.economic == 0.45
    assert dashboard.environment.competitor_pressure == 1.0
    assert dashboard.environment.shock_summary == ['recession', 'currency']
